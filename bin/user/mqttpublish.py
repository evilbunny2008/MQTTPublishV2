...
class MQTTPublish(StdService):
    ...
    def configure_fields(self,
        ...
        if fields_dict:
            for field in fields_dict.sections:
                ...
                group = field_dict.get("group", None)
                if group is not None and weewx.units.obs_group_dict.get(field) is None:
                    self.logger.loginf(f"Added {field} to {group}")
    ...
    def configure_topics(self, service_dict):
        ...
        default_conversion_type = service_dict.get("conversion_type", None)
    ...
    def new_loop_packet(self, event):
        ...
        #self.logger.loginf(f"event.packet: {event.packet}")
...
class PublishWeeWXThread(threading.Thread):
    ...
    def update_record(self, topic_dict, record):
        ...
        topics = topic_dict["fields"]

        #self.logger.loginf(f"topics.keys(): {topics.keys()}")

        for var in updated_record:

            #self.logger.loginf(f"row: {row}")

            for field in topics:

                field_dict = topics[field]

                if field_dict.get("name") != var:
                    continue

                #self.logger.loginf(f"field_dict: {field} -- {field_dict}")

                ignore = field_dict.get("ignore", topic_dict.get("ignore"))
                publish_none_value = field_dict.get("publish_none_value", topic_dict.get("publish_none_value"))

                if ignore:
                    self.logger.loginf(f"Ignoring {field}")
                    continue
    ...
    @staticmethod
    def update_field(logger, topic_dict, field_dict, field, value, unit_system):
        """ Update field. """
        name = field_dict.get("name", field)
        unit = field_dict.get("unit", None)

        append_unit_label = field_dict.get("append_unit_label", topic_dict.get("append_unit_label"))
        ...
        #logger.loginf(f"value: {value}")
        #logger.loginf(f"converted_value: {converted_value}")
